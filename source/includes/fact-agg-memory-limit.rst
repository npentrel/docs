...
default, if a stage exceeds this limit, MongoDB produces an error. To
allow pipeline processing to take up more space, use the
:ref:`allowDiskUse <aggregate-cmd-allowDiskUse>` option to enable
aggregation pipeline stages to write data to temporary files.

If the results of one of your pipeline stages exeed the limit,
consider reducing the byte-size of the output by renaming long field
names or values or removing unnecessary information. If you are using
:pipeline:`$sort`, consider :ref:`adding a $limit stage<sort-limit-sequence>`.

Examples of stages that can exceed the pipeline stage limit are:
